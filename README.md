...
“iCloud 登录”使用 Apple 账户的登录密码，按需输入受信任设备验证码，成功后保存
Cookie 供别名管理使用；App 专用密码用于 IMAP 收信，不能替代此登录。
登录失败会显示对应阶段，验证码错误可在窗口中重试，不会退出管理台。
服务端登录诊断仅记录阶段、类别与状态码，主密码不写入账号配置，调试模式也不输出
Cookie 或请求头。此流程依赖 Apple 的网页登录接口，模拟测试通过不代表每个真实账号
都能登录；受到上游限制时可在 iCloud 官网登录，再通过“更新 Cookie”提供新会话。
...
IMAP 的摘要和详情会解码嵌套 MIME、Base64、quoted-printable 和字符集，优先纯文本正文，
排除附件内容；详情读取使用 BODY.PEEK，保留已读状态。
